        ...
        if in_map {
            ...
            if line.starts_with('[') && line.contains(']') {
                ...
                match key {
                    ...
                    "MSG1" => {
                        // Handle language markers ~SLO~ or ~ENG~
                        if let Some(msg) = extract_language_message(value) {
                            level.messages.push(msg);
                        } else {
...
fn extract_language_message(value: &str) -> Option<String> {
    // Format: ~SLO~message or ~ENG~message
    if let Some(idx) = value.find("~") {
        if idx > 0 {
            let prefix = &value[..idx];
            if prefix == "~SLO" || prefix == "~ENG" {
...
fn print_usage() {
    ...
    println!("    cargo run --bin convert_mie -- ../siriel-levels/FMIS01.MIE assets/levels/fmis01.ron");
