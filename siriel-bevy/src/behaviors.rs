...
impl BehaviorManager {
    ...
    fn platform_with_gravity(
        ...
        if !state.active { // active flag used as "on_ground" here
    ...
    fn random_movement(
        ...
        match state.direction {
            ...
            0 => { velocity.x = 0.0; velocity.y = 0.0; }, // Stop
    ...
    fn fireball(
        ...
        match params.inf1 {
            ...
            3 => { // Move down
                velocity.y = -speed;
                if position.y <= target_x { // inf2 used as target Y
    ...
    fn hunter(
        ...
        if !state.active { // Passive mode
            if distance <= activation_distance && passive_time > 0.0 {
                ...
                if state.timer >= 1.0 { // Change direction every second
                    let mut rng = rand::thread_rng();
                    state.direction = rng.gen_range(-1..=1);
                    state.timer = 0.0;
                }
