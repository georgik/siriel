...
use crate::resources::*;
use crate::audio::{SoundEvent, sound_mappings};
...
pub fn setup_game(
    ...
    commands.spawn((
        Player,
        Position { x: spawn_pos.0, y: spawn_pos.1 },
        Velocity::default(),
        Collider::default(),
        SpriteInfo::default(),
        Physics {
...
pub fn input_system(
    ...
    // Update input state
    input_state.move_left = keyboard_input.pressed(KeyCode::ArrowLeft) || keyboard_input.pressed(KeyCode::KeyA);
    input_state.move_right = keyboard_input.pressed(KeyCode::ArrowRight) || keyboard_input.pressed(KeyCode::KeyD);
    input_state.move_up = keyboard_input.pressed(KeyCode::ArrowUp) || keyboard_input.pressed(KeyCode::KeyW);
    input_state.move_down = keyboard_input.pressed(KeyCode::ArrowDown) || keyboard_input.pressed(KeyCode::KeyS);
...
pub fn collision_system(
    mut player_query: Query<(&mut Position, &Collider, &mut Health), (With<Player>, Without<Behavior>)>,
    ...
    if let Ok((player_pos, player_collider, mut player_health)) = player_query.single_mut() {
        for (entity_pos, entity_collider) in entity_query.iter() {
            ...
            // Check for overlap
            if player_right > entity_left && player_left < entity_right &&
               player_top > entity_bottom && player_bottom < entity_top {
                ...
                if !player_health.invulnerable {
                    ...
                    if player_health.current <= 0 {
                        ...
                        sound_events.send(SoundEvent::PlayEffect(sound_mappings::EXPLOSION.to_string()));
