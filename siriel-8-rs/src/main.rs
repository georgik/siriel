...
fn esp32s3_usb_otg_hello_world(
    ...
    let bmp_data = include_bytes!("siriel-avatar-8bit-00.bmp");
    let bmp_data1 = include_bytes!("siriel-avatar-8bit-01.bmp");
    let bmp_data2 = include_bytes!("siriel-avatar-8bit-02.bmp");
    let bmp_data3 = include_bytes!("siriel-avatar-8bit-03.bmp");
    ...
    let bmp3 = Bmp::<Rgb565>::from_slice(bmp_data3).unwrap();
    ...
    let mut vector = 1;
        ...
        while i > -5 {
            if (image_counter == 0) {
                let im = Image::new(&bmp, Point::new(i, 20));
                im.draw(&mut display)?;
                image_counter = image_counter + 1;
            } else if (image_counter == 1) {
                let im = Image::new(&bmp1, Point::new(i, 20));
                im.draw(&mut display)?;
                image_counter = image_counter + 1;
            } else if (image_counter == 2) {
                let im = Image::new(&bmp2, Point::new(i, 20));
                im.draw(&mut display)?;
                image_counter = image_counter + 1;
            } else if (image_counter == 3) {
                let im = Image::new(&bmp3, Point::new(i, 20));
                im.draw(&mut display)?;
                image_counter = 0;
            }
    ...
    D::Color: From<Rgb565>,
{
    ...
    let r = Rectangle::new(display.bounding_box().top_left, display.bounding_box().size)
        .into_styled(
    ...
    while i > -5 {
        ...
        Text::new(
            "Hello Rust!",
            Point::new(i, (display.bounding_box().size.height - 10) as i32 / 2),
